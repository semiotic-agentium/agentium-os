...
- **Conversation** — `ChatWindow.vue` renders the streaming transcript via `MessageBubble.vue`, surfaces `INPUT_REQUIRED` prompts (`awaitInput`), and shows `RunStatusIndicator.vue` while a task is running.
