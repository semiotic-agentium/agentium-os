...
type SlackInterpretation = {
  executive_summary?: string;
  current_objectives?: string[];
};

type SlackDerivedTask = {
  key?: string;
  title?: string;
  description?: string;
  priority?: string;
...
type SlackSemanticIngressEvent = {
  ingress_kind: "slack_raw_source_records";
  schema_version: string;
  source: SlackSource;
  // Reserved for future project scoping once raw source records carry a stable project identity.
  project: Record<string, never>;
  messages_scanned?: number;
  interpretation: SlackInterpretation;
  derived_tasks: SlackDerivedTask[];
...
const PROJECT_MANAGEMENT_CREATE_TASK_CAPABILITY = "project-management:create-task";
...
function parseDiscoverAgentsOutput(
  value: unknown,
): { agents: DiscoveredAgent[]; done?: boolean } | null {
...
function parseSlackRawSourceRecord(value: unknown): SlackRawSourceRecord | null {
  ...
  return {
    ...
    reply_count: typeof value.reply_count === "number" ? value.reply_count : undefined,
    latest_reply: normalizeOptionalString(value.latest_reply) ?? undefined,
    reply_users: value.reply_users == null ? undefined : parseStringArray(value.reply_users),
...
function parseSlackRawSourceEventValue(value: unknown): SlackSemanticIngressEvent | null {
  if (!isObject(value)) return null;
...
function shouldExpandSlackConversationGroup(group: SlackConversationGroup): boolean {
  return group.records.some((record) => {
    const threadTs = normalizeOptionalString(record.thread_ts);
    const messageTs = normalizeOptionalString(record.ts);
    if (threadTs == null) return false;
    if (threadTs !== messageTs) return true;
    return (
      (record.reply_count ?? 0) > 0 ||
      record.latest_reply != null ||
      (record.reply_users?.length ?? 0) > 0
    );
  });
}
...
async function materializeSlackSemanticIngressEvent(
  event: SlackSemanticIngressEvent,
): Promise<SlackSemanticIngressEvent> {
  ...
  const transcriptLines = renderSlackEventTranscript(
    actionable.length > 0
      ? actionable
      : evaluations.filter((evaluation) => evaluation.transcriptLines.length > 0),
  );
  const expandedConversationCount = evaluations.filter(
    (evaluation) => evaluation.usedThreadExpansion,
  ).length;
  const expandedConversationGroups = evaluations.map((evaluation) => ({
    conversationKey: evaluation.conversationKey,
    records: evaluation.records,
  }));
  const expandedRawSourceRecords = expandedConversationGroups.flatMap((group) => group.records);
  ...
  return {
    ...
    raw_source_records: expandedRawSourceRecords,
    conversation_groups: expandedConversationGroups,
    no_work_reason: derivedTasks.length === 0 ? noWorkReason : undefined,
  };
}

function deriveDecision(event: SlackSemanticIngressEvent): IntakeDecision {
  ...
  return {
    kind: "create_pm_work",
    reason: "Slack raw ingress was interpreted into project-management follow-up work.",
    requiredCapabilities: [PROJECT_MANAGEMENT_CREATE_TASK_CAPABILITY],
  };
}
...
function selectDownstreamAgent(agents: DiscoveredAgent[]): DownstreamSelection {
  ...
  if (agents.length === 1) {
    return { kind: "matched", agent: agents[0] };
  }
  return {
    kind: "ambiguous",
    reason:
...
function renderDerivedTasks(tasks: SlackDerivedTask[]): string[] {
  if (tasks.length === 0) {
...
function renderDownstreamPrompt(
  ...
  lines.push("Project: unscoped-project");
...
async function handleSlackSemanticIngressEvent(
  event: SlackSemanticIngressEvent,
  ...
  if (decision.kind === "noop") {
    return { accepted: true, detail: decision.reason };
  }
  ...
  return {
    accepted: true,
    detail: renderRouteSummary(decision, target, downstreamTexts),
  };
}
