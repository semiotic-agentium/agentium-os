import { executeClickUpPlan, isClickUpIntent, isNeedClarification, isNotRelevant, onClickupSourceDispatch, parseClickUpStructuredPlanFromPlanning, textReply, validateClickUpPlanForExecution, } from "./clickupExecution";
__chat_register({
    run: async (ctx) => {
        ...
        while (true) {
            ...
            return {
                error: "InferClickUpIntent returned an unexpected shape — expected ClickUpIntent, NeedClarification, or NotRelevant.",
            };
        }
        if (!resolvedIntent)
            return { error: "Could not determine a valid ClickUp intent." };
        ...
        if (!structured) {
            return {
                error: "Planning failed: PlanClickUpWork did not return a valid ClickUpStructuredPlan shape. Try rephrasing your request.",
            };
        }
        ...
        return executeClickUpPlan(ctx, structured, resolvedIntent.intent, resolvedIntent.operation_kind, stepsOrErr);
    },
