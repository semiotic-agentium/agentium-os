...
**Host tool implementers** (new Rust tools in `baml-rt-tools`) should read [host-tool-guide.md](../reference/host-tool-guide.md) for registration and contracts; this doc covers how **agents** call those tools and surface results.
...
At publish the host records poll metadata as host `operational_event` rows (visible in operator transcript with `profile=full`). For each `withTask({ unitKey, records })`, the host formats that **record slice** into the unit’s task-scoped ingress prelude before your callback runs. You pass structured `records[]` only; never build duplicate transcript prose for BAML.
...
The host resolves `**SupportCrmSessionPlan` → `support/crm**` and `**SupportEmailSessionPlan` → `support/email**` via tool metadata `class_name`. If the map is stale, the fallback is `**__type**` on the model output (see [host-tool-guide.md](../reference/host-tool-guide.md)).
...
| [host-tool-guide.md](../reference/host-tool-guide.md)                                                            | Adding Rust host tools, manifests, `session_plan_functions.json` / `__type`.  |
