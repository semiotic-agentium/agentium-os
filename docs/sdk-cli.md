...
| `sandbox-bind-sync --tool-dir <dir> [--rootfs <path>] [--image <tag>]` | Generate bind runtime lock + sidecar bundle; can build/export rootfs from Docker |
...
| `--runtime-digest <sha256:...>` | auto | Optional for OCI sandbox. Defaults to OCI digest suffix for `oci`; rejected for `bind` because bind digests are generated into `tool-metadata.lock.json` by `sandbox-bind-sync` |
...
For `sandbox + bind`, scaffolding emits a portable tool-relative rootfs path such as `./.tmp/<bundle>-<tool>-rootfs` and omits `runtime_digest` from `tool-metadata.json`. Host-specific absolute paths and the computed digest live in the gitignored `tool-metadata.lock.json` generated by `sandbox-bind-sync`.
...
- with `--generate-docker`: additionally emits `adapter/Dockerfile` + `adapter/tool-adapter` + `setup_bind_sandbox.sh`; script builds image, exports rootfs, computes digest, writes `tool-metadata.lock.json`, writes adapter sidecar bundle (`/etc/agentium/tool-bundle.json`), and validates metadata.
...
3. Sandbox source consistency checks:
   ...
   - `bind`: source metadata must be portable, lock sidecar must provide the resolved path + digest, and recomputed bind digest must match
...
This is a low-level digest-only helper. Prefer `sandbox-bind-sync` for bind tools because it writes `tool-metadata.lock.json` and the rootfs sidecar bundle together.

---

### `sandbox-bind-sync`

Synchronizes a bind sandbox rootfs with local runtime state. It never mutates committed `tool-metadata.json`; instead it writes the gitignored sibling `tool-metadata.lock.json` and the in-rootfs adapter sidecar at `/etc/agentium/tool-bundle.json`.
