...
For external and sandbox host tools, see [Host tool guide](host-tool-guide.md). MCP pooling, drift handling, and runtime lifecycle are covered in §6–§8 below.
...
| POST | `/mcp/snapshots/import` | Import a server snapshot (operator) |
| POST | `/mcp/servers/{id}/versions/{v}/mark-stale` | Mark a snapshot version stale after runtime drift (operator) |
...
Exported counters/digests hooks include MCP session/registry recreation and digest mismatches (see **`docs/reference/metrics-inventory.md`**, entries prefixed with `mcp.`).
