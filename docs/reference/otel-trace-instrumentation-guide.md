...
Based on Agentium OS crates (`baml-rt-observability`, `baml-rt-core`, `baml-rt-api`): orthogonal span module, structured attributes, async propagation, and testability.
...
- Static span names (`"baml_rt.operation_name"`)
- Namespace prefix prevents collisions (`baml_rt.` vs `sqlx.`)
...
```rust
tracing::info_span!("baml_rt.insert_webhook_event", event_id = eid)
tracing::debug_span!("baml_rt.fetch_effective_rate", currency = cur)
...
tracing::info_span!("baml_rt")  // Too vague
    ...
    pub async fn connect(url: &str, max_connections: u32) -> Result<Self> {
        ...
        let pool = Pool::connect_with(
            sqlx::postgres::PgConnectOptions::from_str(url)?
                .application_name("baml-rt-observability"),
        )
...
```rust
// Your code:
let span = spans::a2a_request(scope.as_ref(), method, agent_package, agent_instance_id, correlation_id);
let _guard = span.enter();

runner.handle_a2a_by_key(&route_key, wire).await?;
```

**Resulting span hierarchy:**

```
baml_rt.a2a_request {method, agent_package, agent_instance_id, correlation_id, context_id, ...}
└── baml_rt.a2a_js_invoke {method, invocation}
```
...
async fn test_ledger_insert_has_db_span() {
    ...
    otel.assert_spans()
        .assert_exists("baml_rt.insert_ledger")
        .assert_child_named("baml_rt.insert_ledger", "sqlx.execute")
        .graph()
...
// Span existence
.assert_exists("baml_rt.insert_ledger")
.assert_not_exists("baml_rt.some_operation")
...
async fn test_webhook_flow_with_spans() {
    ...
    otel.assert_spans()
        .assert_exists("baml_rt.insert_webhook_event")
        .assert_child_named("baml_rt.insert_webhook_event", "sqlx.execute")
        .assert_exists("baml_rt.insert_ledger")
        .assert_child_named("baml_rt.insert_ledger", "sqlx.execute")
...
impl SpanFlowAsserter<'_> {
    ...
    pub fn assert_apply_flow(&self) -> &Self {
        self
            .assert_exists("baml_rt.insert_webhook_event")
            .assert_child_named("baml_rt.insert_webhook_event", "sqlx.execute")
            .assert_exists("baml_rt.duplicate_exists")
            .assert_child_named("baml_rt.duplicate_exists", "sqlx.fetch_optional")
            .assert_exists("baml_rt.insert_ledger")
            .assert_child_named("baml_rt.insert_ledger", "sqlx.execute")
    }
    ...
    pub fn assert_no_apply(&self) -> &Self {
        self
            .assert_not_exists("baml_rt.insert_ledger")
    }
}
```
...
├── baml_rt.insert_webhook_event
│   └── sqlx.execute
├── baml_rt.fetch_webhook_meta
│   └── sqlx.fetch_optional
├── baml_rt.insert_ledger
│   └── sqlx.execute
└── baml_rt.mark_webhook_applied
    └── sqlx.execute
```
...
```rust
tracing::info_span!("baml_rt.insert_webhook", event_id = event_id);  // ✅
```
...
async fn test_full_flow() {
    ...
    otel.assert_spans()
        .assert_exists("baml_rt.insert_webhook_event")
        .assert_child_named("baml_rt.insert_webhook_event", "sqlx.execute")
        .assert_exists("baml_rt.apply_transaction")
        .assert_child_named("baml_rt.apply_transaction", "sqlx.execute");
}
```
