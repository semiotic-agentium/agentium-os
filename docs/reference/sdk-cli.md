...
Synchronizes a local-development bind sandbox rootfs with runtime state. It never mutates committed `tool-manifest.json`; instead it writes the gitignored sibling `tool-manifest.lock.json` with the host-resolved rootfs path and writes the in-rootfs adapter sidecar at `/etc/agentium/tool-bundle.json`.
