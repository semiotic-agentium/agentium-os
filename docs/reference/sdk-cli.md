...
**Cluster mode** on the runner is enabled by **`--surreal-endpoint`** (shared SurrealDB + registry), not by setting `RUNNER_TOKEN` alone. In standalone mode, operator routes are open when no token is configured; when a token is set, the same operator routes require authentication.

Operator actions (publish, deploy, undeploy, push, and MCP registry mutations such as `mcp enable`) send `X-Runner-Token` when a token is configured. The token is set on the runner via `RUNNER_TOKEN` env or `--runner-token` flag (see `docs/reference/agent-runner.md`).
...
Generated scaffold always includes `tool-manifest.json`, `README.md`, and language-specific files. For `runtime=process`, language templates include `tool-server`, which is the host executable the runner invokes. For `runtime=sandbox`, non-Bash scaffolds omit host `tool-server`; runner invocation goes through `/tool-adapter` inside the sandbox. Bash sandbox scaffolds still include `tool-server` because it is the Bash implementation script that the adapter delegates to. `tool-manifest.json` always emits an explicit `runtime` block (`process` by default, or `sandbox` when selected).

**Schema ownership:** `new-tool` does **not** emit `tool-schema.json` or embed input/output schemas in `tool-manifest.json`. Starters embed a trivial echo schema in the language source and implement `tool/schema` plus `schema_digest` on `tool/describe`. The runner discovers schema live from the running tool (`tool/describe` → `tool/schema`); approved snapshots store the discovered contract for builder/registry projection. You may externalize schema to a file in your own implementation (for example `examples/external-tools/meteo-tool/tool-schema.json`), but that is author choice, not scaffold output.
...
For `sandbox + bind`, scaffolding emits a portable tool-relative rootfs path such as `./.tmp/<bundle>-<tool>-rootfs`. Bind is a local development convenience; host-specific absolute paths live in the gitignored `tool-manifest.lock.json` generated by `sandbox-bind-sync`.

Bind scaffold modes:
- default (no `--generate-docker`): manifest-only bind scaffold; materialize rootfs externally, then run `sandbox-bind-sync` to generate `tool-manifest.lock.json`, write the adapter sidecar bundle, and validate.
- with `--generate-docker`: additionally emits `adapter/Dockerfile` + `adapter/tool-adapter` + `setup_bind_sandbox.sh`; script builds image, exports rootfs, writes `tool-manifest.lock.json`, writes adapter sidecar bundle (`/etc/agentium/tool-bundle.json`), and validates manifest.

**`tool-bundle.json` (sandbox sidecar):** written into the guest at `/etc/agentium/tool-bundle.json`. Contains `runtime` (child `command`, `workdir`, `protocol`) and `manifest` (`tool_name`, `protocol_version`, `supported_methods`) so the generated shim can launch the real tool. It does **not** carry schema; the shim forwards `tool/describe`, `tool/schema`, and `tool/invoke` to the child, which is the sole schema source.
...
Validates a standalone external tool's `tool-manifest.json` against:

1. The runtime typed parser (`ExternalToolManifest`)
2. Sandbox source consistency checks:
   - `oci`: image must be digest-pinned (`repo@sha256:...`)
   - `bind`: source manifest must be portable and the lock sidecar, when present, provides the resolved local path
...
| Option | Default | Description |
|--------|---------|-------------|
| `--path <dir>` | `.` | Tool directory containing `tool-manifest.json` |

---

### `sandbox-bind-sync`

Synchronizes a local-development bind sandbox rootfs with runtime state. It never mutates committed `tool-manifest.json`; instead it writes the gitignored sibling `tool-manifest.lock.json` with the host-resolved rootfs path and writes the in-rootfs adapter sidecar at `/etc/agentium/tool-bundle.json` (`runtime` + `manifest` only — no schema).

```bash
# Existing rootfs mode: --rootfs defaults to runtime.image.path from manifest.
...
| Option | Default | Description |
|--------|---------|-------------|
| `--tool-dir <dir>` | *(required)* | Tool directory containing `tool-manifest.json` |
...
| "authentication required" on publish/deploy/undeploy | Operator route without token while runner has `RUNNER_TOKEN` configured — pass `--runner-token` or set `RUNNER_TOKEN` |
...
- `docs/assertions/host-to-agent-event-delivery.md` — Event delivery model and subscriptions
- `docs/reference/agent-runner.md` — Running and managing the agent runner
