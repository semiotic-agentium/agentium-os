...
**What happens at runtime:** On the **select** phase, the generated `ExecuteStep__select` function narrows the return type to `**CrmStepResult | SupportCrmOpenStep | SupportEmailOpenStep**`. The **narrowed return type** is what the BAML parser enforces; the prompt adds a **system-role stable prelude** (`ctx.tags['tool_schema_prelude']`) with tool cards and class definitions, then the parent `prompt_template` (with standalone `{{ ctx.output_format }}` lines removed), then a compact **phase footer** listing allowed variants for this hop. The model picks a tool by emitting, for example:
...
Each generated phase prompt places manifest tool/session BAML once in a **system** block, then the parent task/history template, then a **footer** with notation for **only** the legal variants on this hop (matching the phase function's return union). The model literally cannot express an illegal transition at parse time because the return type omits disallowed ops.
...
**Tool corpus, transcript order, and spare prose (session and step-executor BAML).** The merged `baml_src/_baml_runtime.baml` holds **all** tool cards, `*OpenInput`, `*SendInput`, `ArchiveSearchReadInput` / `ArchivePageReadInput`, and step classes. Field names and semantics live in those types and in `@@description` — not in a second, hand-copied “JSON with `query` / `limit` / …” block in the `prompt` text. **Generated** per-phase functions (`__select` / `__act__` / `__continue__*`) order the prompt as: `{{ _.role('system') }}` → `{{ ctx.tags['tool_schema_prelude'] }}` → `{{ ctx.tags['session_step_stable_prefix'] }}` (archive read-before-repeat policy; set on `[invoke_function_with_intra](../crates/baml-rt-quickjs/src/baml/intra_turn.rs)` only) → parent IR `prompt_template` with **standalone** `{{ ctx.output_format }}` **lines stripped** → `---` and a **phase union footer** (compact notation for legal variants on this hop). Enforcement remains the **narrowed return type** for that phase function. Hand-written **umbrella** session-plan `prompt` bodies may still use **task** → transcript → `{{ ctx.output_format }}` when you rely on the umbrella return type for a direct call. **FSM parent** templates copied into generated phase functions should **omit** standalone `{{ ctx.output_format }}` lines (codegen strips them anyway); use prelude + phase footer + narrowed type instead. Plain plan/synthesis functions (non-session) may keep `output_format` last for readability. Do not add a parallel FSM story in prose that could disagree with the generated union.
...
Canonical reference implementation: **[conversational-persona-demo](../tests/fixtures/agents/conversational-persona-demo/)** — intent before discovery, `**MakePersonaStructuredPlan`** with model-authored `**step_id` / `depends_on**`, `**ObserveAndRevisePersonaPlan**` after delegations, `**PersonaReact**` for the single operator-visible `**StructuredReply**`.
...
**Do instead:** Loop until you get a resolved branch (e.g. **`SlackIntent`** / **`NotRelevant`**) or an unexpected shape you surface as **`{ error }`**. On every **`NeedClarification`**, **`await ctx.emit.awaitInput(...)`** then **`continue`** with the clarified text — same pattern as [task-lifecycle-demo](../tests/fixtures/agents/task-lifecycle-demo/src/index.ts)’s suspension rail. Product agents such as [slack-agent](../agents/slack-agent/src/index.ts), [notion-agent](../agents/notion-agent/src/index.ts), and [clickup-agent](../agents/clickup-agent/src/index.ts) follow this.
