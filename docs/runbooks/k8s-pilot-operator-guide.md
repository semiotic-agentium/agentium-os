...
| Runner pod restarts with exit code 101 and log line `fatal: unhandled panic` | `kubectl -n agentium logs <pod> --previous`; inspect `storage_panic=` and `site=` fields | Runner fail-fast hook terminated process after an unhandled panic. Treat `storage_panic=true` as a storage/WAL/disk-pressure hint, not proof; kubelet restart is expected recovery path. |
