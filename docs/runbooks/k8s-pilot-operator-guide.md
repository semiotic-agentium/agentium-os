...
| Runner pod restarts after structural task panic/error | `kubectl -n agentium logs <pod> --previous`; inspect `HTTP API listener task panicked`, `event poll loop panicked`, or task failure logs | Runner returned a fatal error from a structural task. Kubelet restart is expected recovery path. HTTP request handler panics are isolated and return 500 without restarting the pod. |
