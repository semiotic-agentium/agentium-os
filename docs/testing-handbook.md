...
|----------|-------------------------|---------------------|
| Semantic-ingress raw-source dispatch batching | `baml-agent-runner/tests/runner_semantic_ingress_test.rs` — `semantic_ingress_dispatch_http_batches_actionable_threads_into_one_downstream_route` | `runner_workflow_intake_test.rs` or other runner tests re-covering the same semantic-ingress routing/batching contract |
