...
- **Run status strip** — Persistent phase label (`Validating` → `Publishing` → `Recording` → `Live`) with a thin in-flight progress bar. Publish acceptance summary and subscriber failures stay visible until dismissed or superseded; no traffic-light stepper or auto-hiding success panel.
