...
| --- | --- | --- | --- | --- |
| CLI startup | `src/main.rs`, `Cargo.toml` | **MIGRATED 2026-06-23:** removed `baml-tool-links` dependency, removed `baml_tool_links::force_link_all_tools!()`, removed crate-level `#![allow(unexpected_cfgs)]` | no local inventory blocker found | keep `crates/baml-tool-links` crate in workspace for dev/static-tool patcher compatibility, but release CLI no longer depends on it |
...
| `doctor` | `src/commands/doctor.rs` | **MIGRATED 2026-06-23:** file checks no longer inspect `baml-tool-links`; catalog checks use `--repository-url` (`/tools`) or `--snapshot-cache`; no local `InventoryCatalog` | no local inventory blocker found | keep no-source behavior explicit: catalog checks skipped only with `--warn-missing-catalog`, otherwise error |
...
Conclusion: default `cargo-agent-platform` no longer depends on `baml-tool-links` or force-links static tools. Remaining references are dev/internal patchers for `new-static-tool`; keep `crates/baml-tool-links` in workspace as orphaned compatibility/developer scaffolding for now.
...
- [x] Audit ALL `InventoryCatalog`/`force_link_all_tools` consumers in `cargo-agent-platform` (not just regen) before removing the dep (Finding 6). Findings captured under “Phase 7 audit results (2026-06-23)”; blockers remain in startup macro/dep and dev patchers.
...
- [x] Migrate `doctor` off default local `InventoryCatalog`/`baml-tool-links`: static checks are file/workspace-only; catalog validation uses `--repository-url` or `--snapshot-cache`; no-source catalog validation is explicit skip/error based on `--warn-missing-catalog`.
- [x] Remove `baml-tool-links` from default `cargo-agent-platform` release path: dependency removed, startup macro removed, `#![allow(unexpected_cfgs)]` removed. `crates/baml-tool-links` intentionally kept in workspace for orphaned/dev patcher compatibility.
- [x] Gate or remove `baml-tool-links` dependency from release CLI. Removed from `crates/cargo-agent-platform/Cargo.toml`; crate remains in workspace for dev/static-tool scaffolding.
- [x] Remove unconditional `force_link_all_tools!()` from release CLI startup.
