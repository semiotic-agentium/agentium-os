...
fn skip_field_absent_in_ts() {
    ...
    assert!(!decl.contains("internal_cache"), "skipped field must not appear in TS: {decl}");
...
fn alias_does_not_change_ts_field_name() {
    ...
    assert!(decl.contains("task_id"), "TS should use the Rust field name: {decl}");
    assert!(
...
fn unit_enum_skip_variant_ts() {
    ...
    assert!(!decl.contains("Internal"), "skipped variant must not appear in TS: {decl}");
...
fn render_ts_declarations_joins_decls() {
    ...
    let decls = vec![
        TsStatus::ts_decl(),
        TsSimpleInput::ts_decl(),
    ];
    let output = render_ts_declarations(&decls);
    assert!(output.starts_with("// Auto-generated by baml-derive"), "missing header");
    assert!(output.contains("export type TsStatus"), "missing enum decl");
    assert!(output.contains("export interface TsSimpleInput"), "missing interface decl");
}
