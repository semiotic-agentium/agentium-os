...
/// Coerce a row column to String. GraphQLite can return node properties as different types;
/// we use the vocabulary's strict type (String) for task id so read/write match.
fn row_get_string(row: &GraphRow, col: &str) -> Option<String> {
    row.get::<String>(col)
        .ok()
        .or_else(|| row.get::<i64>(col).map(|n| n.to_string()).ok())
        .or_else(|| row.get::<f64>(col).map(|n| n.to_string()).ok())
}

fn decode_task_node(row: &GraphRow) -> Option<TaskSubgraphNode> {
    Some(TaskSubgraphNode {
        id: row_get_string(row, "id")?,
        context_id: row_get_string(row, "context_id").unwrap_or_default(),
...
impl A2aGraphStore for GraphqliteProvenanceStore {
    ...
    async fn get_task_node(&self, id: &str) -> A2aGraphStoreResult<Option<TaskSubgraphNode>> {
        // Use strict type: id as escaped literal so read matches write (GraphQLite MERGE pattern
        // does not resolve $param; use literal for identity so MATCH finds the node).
        let id_escaped = escape_string(id);
        let query = format!(
            "MATCH (t:{TASK_NODE_LABEL}) WHERE t.id = '{id_escaped}' \
        ...
        let rows = self
            .run_cypher_read(&query, &empty_params())
            .await
            .map_err(|e| e.to_string())?;
    ...
    async fn upsert_task_node(
        ...
        // Use strict type: id as escaped literal in MERGE pattern (extension does not resolve $param
        // in pattern; see tool_index.rs). SET clause still uses params.
        let id_escaped = escape_string(&node.id);
        let query = format!(
            "MERGE (t:{TASK_NODE_LABEL} {{id: '{id_escaped}'}}) \
             ON CREATE SET t.ord = $ord, t.context_id = $context_id, t.status_json = $status_json, \
                 t.metadata_json = $metadata_json, t.extra_json = $extra_json, t.artifacts_json = $artifacts_json \
