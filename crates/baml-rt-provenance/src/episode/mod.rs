//! Historic **episode** view for a completed `(context_id, task_id)`: assembled from the
//! provenance graph and rendered as grep-friendly text with episode-local, namespaced refs
//! (`XXXX#N` / `XXXX@N`) so they never collide with a live agent `#N`/`@N` table.
...
pub use renderer::{prefix_wire_citation, render_episode};
...
/// Flat shape without preview text (belongs in explore/ops, not episodic summary).
#[derive(Debug, Clone, Serialize, Deserialize)]
