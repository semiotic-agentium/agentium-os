...
impl MermaidServiceImpl {
    ...
    async fn export_by_context_family(
        &self,
        context_id: &str,
    ) -> std::result::Result<baml_rt_provenance::ExportedGraph, baml_rt_api::MermaidError> {
        let exporter = GraphExporter::new(self.store.clone());
...
impl baml_rt_api::MermaidService for MermaidServiceImpl {
    ...
    async fn mermaid_for_context_full(
        ...
        let cache_key = format!("{context_id}::full");
        if let Some(ref cache) = self.cache
            && let Some(cached) = cache.get(&cache_key)
        {
            tracing::debug!(context_id = %context_id, "mermaid full context: cache HIT");
            return Ok(cached);
        }
        let t0 = std::time::Instant::now();
        let graph = self.export_by_context_family(context_id).await?;
        if graph.nodes.is_empty() {
            return Err(baml_rt_api::MermaidError::NotFound);
        }
        let t1 = std::time::Instant::now();
        ...
        tracing::debug!(
            ...
            simplified_edges = simplified.edges.len(),
            bytes = mermaid.len(),
            "mermaid full context export"
        );
        if let Some(ref cache) = self.cache {
            cache.insert(&cache_key, mermaid.clone());
        }
        Ok(mermaid)
    }
