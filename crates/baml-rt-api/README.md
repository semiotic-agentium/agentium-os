...
flowchart TB
  ...
  subgraph Trait["AgentRegistry (trait)"]
    ...
    A2a["handle_a2a(key, request) → Result<Vec<Value>>"]
  end
...
| **POST** | `/agents/{agent_package}/{agent_instance_id}/a2a` | A2A JSON-RPC request. Body: single JSON-RPC 2.0 object. Response: JSON array of JSON-RPC 2.0 responses. |
| **POST** | `/agents/{agent_package}/{agent_instance_id}/a2a/sse` | Same as above; response is **Server-Sent Events** (`text/event-stream`). One event per JSON-RPC response; 15s keep-alive. |
