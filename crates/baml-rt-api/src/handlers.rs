...
pub async fn post_undeploy(
    ...
    let result = async {
        ...
        let undeploy_result = run_off_worker("undeploy", move || async move {
...
pub async fn get_deployments(
    ...
    let result = async {
        ...
        let manager_clone = Arc::clone(manager);
        let deployments_result = run_off_worker("list_deployments", move || async move {
            manager_clone.list_deployments().await
        })
        .await?;
...
pub async fn post_migrate(
    ...
    let result = async {
        ...
        let manager_clone = Arc::clone(manager);
        let undeploy_hash = content_hash.clone();
        let undeploy_result = run_off_worker("migrate_undeploy", move || async move {
            manager_clone.undeploy_by_hash(&undeploy_hash).await
        })
        .await?;
...
pub async fn post_a2a(
    ...
    // to reach the info-level identity span or the agent-scoped metric recorder.
    // Parse failures fall back to the route-only `record_request` helper so
    // `/agents/...` public traffic cannot spray unbounded labels.
...
pub async fn post_dispatch(
    ...
    // Low-cardinality rule: see `post_a2a`. Parse failures must not reach the
    // identity span or the agent-scoped recorder.
