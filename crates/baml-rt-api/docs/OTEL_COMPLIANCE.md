...
This crate follows the patterns in `otel-trace-instrumentation-guide.md` and `otel-metrics-instrumentation-guide.md`.

## Trace (spans)

- **Orthogonal spans module**: `src/spans.rs` defines span helpers; handlers do not use `#[tracing::instrument]`.
- **Static span names**: `baml_rt_api.list_agents`, `baml_rt_api.post_a2a`, `baml_rt_api.post_a2a_sse` (namespace prefix, low cardinality).
...
- **Structured attributes**: `route` (list_agents, post_a2a, post_a2a_sse) and `result` (success, error); low cardinality.
