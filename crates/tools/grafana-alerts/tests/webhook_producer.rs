...
#[tokio::test]
async fn enqueue_then_poll_emits_grafana_alert_event() {
    ...
    let outcome = enqueue_webhook(&p, &mapping, store.as_ref(), DEFAULT_SOURCE_KEY, 1_700_000_000)
        .await
        .expect("enqueue");
...
#[tokio::test]
async fn repeated_firing_reuses_context_id_across_polls() {
    let _guard = test_lock().lock().unwrap();
    let (_store_guard, store) = install_memory_ingress_store();
    let mapping = MappingStore::open_in_memory().expect("mapping store");
    let producer = GrafanaAlertEventProducer::new().expect("producer");
...
#[tokio::test]
async fn resolved_then_new_firing_mints_fresh_context() {
    let _guard = test_lock().lock().unwrap();
    let (_store_guard, store) = install_memory_ingress_store();
    let mapping = MappingStore::open_in_memory().expect("mapping store");
    let producer = GrafanaAlertEventProducer::new().expect("producer");
    ...
    let resolved_ctx = poll_resolved.events[0]
        .context_id
        .as_ref()
        .unwrap()
        .clone();
...
#[tokio::test]
async fn ingress_enqueue_is_idempotent_per_message_id() {
    let _guard = test_lock().lock().unwrap();
    let (_store_guard, store) = install_memory_ingress_store();
    let mapping = MappingStore::open_in_memory().expect("mapping store");
