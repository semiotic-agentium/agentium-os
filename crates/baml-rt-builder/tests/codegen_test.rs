...
/// Omit `derivedFromMessageIds` on the wire; Rust host must still resolve planning (scope backfill).
#[tokio::test]
async fn test_generated_a2a_shim_submit_intent_omits_lineage_host_backfills() {
    let agent_id =
        AgentId::from_uuid(UuidId::parse_str("00000000-0000-0000-0000-00000000bb12").unwrap());
    let (mut bridge, capture) = make_capturing_bridge(agent_id.clone()).await;
    ...
    let context_id = ContextId::new(7_003, 1);
    let message_id = MessageId::from_external(ExternalId::new("msg-shim-omit-lineage"));
    let task_id = TaskId::from_external(ExternalId::new("task-shim-omit-1"));
    let scope = RuntimeScope::task_scope(context_id, agent_id, message_id, task_id);
    let invoke_scope = InvocationScope::new(scope.clone());

    let js = r#"
      (async function() {
        const session = await openA2aExecutionSession("tok-generated-shim-omit");
        await session.submitIntent({
          intentId: "intent-shim-omit",
          description: "Intent without derivedFromMessageIds on wire"
        });
        await session.submitPlan({
          intentId: "intent-shim-omit",
          planId: "plan-shim-omit",
          steps: [
            { stepId: "step-shim-omit", description: "One step", order: 0, dependsOn: [] }
          ]
        });
        await session.startStep("step-shim-omit", "start");
        await session.completeStep("step-shim-omit", "done");
        return { ok: true };
      })()
    "#;
    ...
    let result = context::with_scope(scope, async {
        bridge
            .eval_scoped(&invoke_scope, js)
            .await
            .expect("run execution session with omitted lineage")
    })
    .await;
    assert_eq!(result.get("ok").and_then(Value::as_bool), Some(true));

    let events = capture.events.lock().await.clone();
    let intent_evt = events.iter().find_map(|e| match e {
        EffectEvent::IntentResolved {
            intent_id,
            derived_from_message_ids,
            ..
        } if intent_id.as_str() == "intent-shim-omit" => Some(derived_from_message_ids.clone()),
        _ => None,
    });
    assert_eq!(
        intent_evt.as_deref(),
        Some(&["msg-shim-omit-lineage".to_string()][..]),
        "host should backfill derived_from_message_ids from invocation scope; events={events:?}"
    );
}
