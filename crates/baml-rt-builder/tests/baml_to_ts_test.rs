...
// ---------------------------------------------------------------------------
// Session plan manifest: direct return type finds session plan
// ---------------------------------------------------------------------------

#[tokio::test]
async fn session_plan_manifest_direct_return_type() {
    use baml_rt_builder::builder::baml_signature_gen::session_plan_functions_map;
    ...
    let map = session_plan_functions_map(&ir_signature);
    assert_eq!(
        map.get("ChooseCalcTool").map(String::as_str),
        Some("SupportCalculateSessionPlan"),
        "ChooseCalcTool -> SupportCalculateSessionPlan should be in manifest; got: {map:?}"
    );
}

// ---------------------------------------------------------------------------
// Session plan manifest: union return type (FinalResponse | SessionPlan)
// ---------------------------------------------------------------------------

#[cfg(feature = "clickup")]
#[tokio::test]
async fn session_plan_manifest_union_return_type() {
    use baml_rt_builder::builder::baml_signature_gen::session_plan_functions_map;

    // The clickup agent's generated_tools.baml is regenerated by the builder
    // to type-alias format. Simulate that by writing the type-alias BAML, loading
    // the runtime, and checking the manifest.
    let agent_dir = workspace_root().join("agents").join("clickup-agent");
    let baml_src = agent_dir.join("baml_src");
    if !baml_src.exists() {
        return;
    }

    // Regenerate generated_tools.baml with type-alias format (same as builder does)
    let tool_names = vec![
    ...
    let baml_interfaces =
        baml_rt_builder::builder::baml_gen::render_baml_tool_interfaces(&tool_names)
            .expect("render BAML interfaces");
    let gen_path = baml_src.join("generated_tools.baml");
    let original = std::fs::read_to_string(&gen_path).ok();
    std::fs::write(&gen_path, &baml_interfaces).expect("write generated_tools.baml");

    let env_vars: HashMap<String, String> = HashMap::new();
    let result = BamlRuntime::from_directory(&baml_src, env_vars, FeatureFlags::default());

    // Restore original file regardless of test outcome
    if let Some(orig) = original {
        std::fs::write(&gen_path, orig).expect("restore generated_tools.baml");
    }

    let runtime = result.expect("load clickup runtime");
    let ir_signature = extract_baml_signatures(&runtime).expect("extract signatures");
    let map = session_plan_functions_map(&ir_signature);

    assert!(
        map.get("ChooseClickUpAction")
            .map(|v| v.contains("SupportClickup"))
            .unwrap_or(false),
        "ChooseClickUpAction should map to SupportClickupSessionPlan; got: {map:?}"
    );
}
